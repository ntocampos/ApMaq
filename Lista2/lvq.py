...
def LVQ1(filename):
    ...
    return prototypes_set, prototypes_set_original, test_set
...
def LVQ21(filename):
    ...
    iterations = 50 # Number of adjustment iterations
    # Array of alphas
    alphas_array = list(reversed(np.linspace(0.1, 0.9, iterations).tolist()))
...
def main():
    ...
    for filename in filenames:
        proto, proto_original, test = LVQ1(filename)
        print "# Test with prototypes #"
        runTests(proto, test, 1)

        print "# Test with original prototypes #"
        runTests(proto_original, test, 1)
